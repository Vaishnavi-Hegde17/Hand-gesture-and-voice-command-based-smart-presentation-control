...
def start_system():
    ...
    while True:
        ...
        if landmarks:
            ...
            cv2.putText(frame, "Recommended hand distance:", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            y_offset += spacing
            cv2.putText(frame, "15 to 50 cm from the camera", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 255, 200), 1)
            y_offset += spacing
            cv2.putText(frame, f"Current: {int(dist)} px", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 1)
            y_offset += spacing
            if dist < 10:
                cv2.putText(frame, "Too far move hand closer", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            elif dist > 300:
                cv2.putText(frame, "Too close move hand farther", (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        ...
        if landmarks and is_valid_hand_distance(landmarks):
            ...
            if gesture_key == current_gesture and action:
                ...
                if elapsed >= hold_duration and gesture_confirmed != gesture_key:
                    ...
                    cv2.putText(frame, f"Holding: {action} ({elapsed:.1f}s)", (10, 160), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            ...
            if action:
                cv2.putText(frame, f"Detected: {action}", (10, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
            ...
            gesture_start_time = None
            gesture_confirmed = None
            cv2.putText(frame, "Show gesture in frame", (10, 160), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)

        cv2.putText(frame, "Press 'c' to Close", (10, 470), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 200), 1)
        ...
        if not pinned:
            try:
                win = gw.getWindowsWithTitle("Smart Presentation Controller")[0]
                win32gui.SetWindowPos(win._hWnd, win32con.HWND_TOPMOST, 0, 0, 0, 0,
